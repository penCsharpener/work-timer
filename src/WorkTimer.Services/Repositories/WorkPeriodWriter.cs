...
namespace WorkTimer.Repositories {
    public class WorkPeriodWriter : IWorkPeriodWriter {
        ...
        private const string FullUpdateQuery = "UPDATE `WorkPeriods` SET `StartTime`=@startTime, `EndTime`=@endTime, `IsBreak`=@isbreak, `Comment`=@comment, `ExpectedHours`=@expectedHours WHERE _rowid_=@id;";
        private const string InsertQuery = "INSERT INTO `WorkPeriods`(`StartTime`,`EndTime`,`IsBreak`,`Comment`, `ExpectedHours`) VALUES (@StartTime, @EndTime, @IsBreak, @Comment, @ExpectedHours);";
        ...
        public async Task<WorkPeriod> Update(int id, DateTime startTime, DateTime? endTime, bool isBreak, string? comment, int expectedHours) {
            var p = new {
                id,
